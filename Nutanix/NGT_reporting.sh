...
#
# Note From Max 
# This script my break Q4 2026 when the legacy APIs are retired v2 & v3
# Script Adjusted from 
#
directory="/Users/max/Downloads"
#
# Specify Nutanix Cluster FQDN, User and Password
clusterfqdn="000.000.000.000"
user="username"
passwd="ILeakAPIKeysInChatGPT"
#
#-------------------------------------------
...
#
# Find cluster name
clustername=`echo $getcluster | python3 -m json.tool | grep -A 1 multicluster | grep -w name | awk -F "\"" '{print $4}'`
#
# Get date
...
#
# Get VM uuids
vmuuids=`echo $getvms | python3 -m json.tool | grep -w uuid | awk -F":" '{print $2}' | awk -F"\"" '{print $2}'`
#
# Create the report
for i in $vmuuids ;
    do
        ...
        # Get VM name
        vmname=`echo $vminfo | python3 -m json.tool |grep -B 2 resources | grep -m 1 name | awk -F "\"" '{print $4}'`
        #
        # Output display message
        echo "Creating reporting input for VM $vmname now ....."
        # Get NGT Info
        ngtstat=`echo $vminfo | python3 -m json.tool |grep -w -m 1 state | awk -F "\"" '{print $4}' | awk '{print tolower($0)}'`
        # Get VM NGT Status
        ...
        #echo $ngtstatus
        # Get NGT version
        ngtver=`echo $vminfo | python3 -m json.tool | grep -w -m 1 version | awk -F "\"" '{print $4}'`
        #ngtver=`echo $vminfo | python3 -m json.tool |grep -w version | awk -F "\"" '{print $4}'`
        ...
        # Get NGT ISO info
        ngtisoinfo=`echo $vminfo | python3 -m json.tool |grep -w -m 1 iso_mount_state | awk -F "\"" '{print $4}' | awk '{print tolower($0)}'`
        #echo $ngtisoinfo
        ...
        #echo $ngtisomounted
        # Get NGT Services
        ngtfeatures=`echo $vminfo | python3 -m json.tool | grep -A 2 -i enabled_capability_list | awk -F "\"" '{print $2}' | grep -v enabled_capability_list | awk 'FNR > 2  {print $0}' | awk '{print tolower($0)}'`
        #echo $ngtfeatures
        ...
        ngtcommunication=`echo $vminfo | python3 -m json.tool |grep -w is_reachable  | awk -F "\ " '{print $2}' | tr -d ','`
